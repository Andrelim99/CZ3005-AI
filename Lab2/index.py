...
class WumpusWorld:
    def __init__(self):
        ...
        self.walls = []
        self.walls += [(i, 0) for i in range(self.world_size[0])] + [(i, self.world_size[1] - 1) for i in
                                                                     range(self.world_size[0])]
        self.walls += [(0, i) for i in range(self.world_size[1])] + [(self.world_size[0] - 1, i) for i in
    ...
    def take_action(self, action):
        ...
        list(self.db.query(
            f"set_position(position({self.agent_position[0]},{self.agent_position[1]},{self.agent_orientation}))"))
        list(self.db.query(
            f"set_senses(senses({str(self.agent_senses.confounded).lower()},{str(self.agent_senses.stench).lower()},{str(self.agent_senses.tingle).lower()},{str(self.agent_senses.glitter).lower()},{str(self.agent_senses.bump).lower()},{str(self.agent_senses.scream).lower()}))"))
    ...
    def update_agent_position_and_senses(self, new_position):
        ...
        for x, y in relative_neighbors:
            ...
            # out of bounds
            if neighbor_x < 0 or neighbor_x > self.world_size[1] - 1 or neighbor_y < 0 or neighbor_y > self.world_size[
                0] - 1:
                continue
